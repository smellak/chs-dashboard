...
export function StoreTable({ stores, title = "Rendimiento por Tienda" }: StoreTableProps) {
  ...
  const totalMargen = stores.reduce((s, r) => s + r.margenReal, 0);
          ...
          <tbody>
            {stores.map((store) => (
              <tr key={store.nombre} className="border-t border-[var(--chs-border-light)] hover:bg-[var(--chs-bg)] transition-colors">
                ...
                <td className="px-4 py-2.5 text-right tabular-nums">{fmtNum(store.margenReal)} €</td>
                <td className="px-4 py-2.5 text-right tabular-nums font-medium">{store.mbPct.toFixed(1).replace(".", ",")}%</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="border-t-2 border-[var(--chs-border)] bg-[var(--chs-bg)] font-semibold">
              ...
              <td className="px-4 py-3 text-right tabular-nums">{fmtNum(totalMargen)} €</td>
              <td className="px-4 py-3 text-right tabular-nums">{totalReal > 0 ? ((totalMargen / totalReal) * 100).toFixed(1).replace(".", ",") : "0,0"}%</td>
            </tr>
          </tfoot>
        </table>
      </div>
