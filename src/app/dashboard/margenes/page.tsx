import { getResumenEmpresa, getDatosCategorias, getDatosTiendas, getLatestPeriod } from "@/lib/queries/ventas";
...
export default async function MargenesPage() {
  ...
  const hasObjetivos = resumen.margenObjetivo > 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <KPICard
          ...
          sub={hasObjetivos ? `Obj: ${fmtK(resumen.margenObjetivo)} €` : `Año ant: ${fmtK(resumen.margenAnterior)} €`}
          trend={hasObjetivos ? ((resumen.margenReal / resumen.margenObjetivo) * 100) - 100 : (resumen.margenAnterior > 0 ? ((resumen.margenReal - resumen.margenAnterior) / resumen.margenAnterior) * 100 : 0)}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-4">
        {categorias.filter((c) => c.ventasReal !== 0).map((cat) => {
          const catHasObj = cat.margenObjetivo > 0;
          return (
            <div
              key={cat.codigo}
